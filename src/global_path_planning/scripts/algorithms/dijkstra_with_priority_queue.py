...
def dijkstra_with_heap(start_index, goal_index, width, height, costmap, resolution, origin, grid_viz, previous_plan_variables):
    open_list = []
    closed_list = set()
    parents = dict()
    g_costs = dict()
    g_costs[start_index] = 0
    heapq.heappush(open_list, [0, start_index])
    shortest_path = []

    path_found = False
    rospy.loginfo('Dijkstra: initialization has been done with priority queue')
        ...
        for neighbor_index, step_cost in neighbors:
            ...
            if in_open_list:
                if g_cost < g_costs[neighbor_index]:
                    ...
                    open_list[idx] = [g_cost, neighbor_index]

            else:
                g_costs[neighbor_index] = g_cost
                parents[neighbor_index] = current_node
                heapq.heappush(open_list, [g_cost, neighbor_index])
